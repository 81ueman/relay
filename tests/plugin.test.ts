...
import {
  ...
  socketPathFor,
  type AttachState,
} from "../.opencode/plugins/agentctl";

// Plugin-side routing + attach correctness:
//   - a KNOWN session directory never routes to another project's AGENTCTL_SOCK
//   - AGENTCTL_SOCK is only a fallback when the directory is unknown
...
const SAVED_SOCK = process.env.AGENTCTL_SOCK;
const SAVED_DEDICATED = process.env.AGENTCTL_DEDICATED;

let root = "";
beforeEach(() => {
  root = mkdtempSync(join(tmpdir(), "relay-plugin-"));
  delete process.env.AGENTCTL_DEDICATED;
});
afterEach(() => {
  if (SAVED_SOCK === undefined) delete process.env.AGENTCTL_SOCK;
  else process.env.AGENTCTL_SOCK = SAVED_SOCK;
  if (SAVED_DEDICATED === undefined) delete process.env.AGENTCTL_DEDICATED;
  else process.env.AGENTCTL_DEDICATED = SAVED_DEDICATED;
...
describe("per-session socket routing", () => {
  test("known directory never routes to another project's AGENTCTL_SOCK", () => {
    const projA = join(root, "project-A");
    const projB = join(root, "project-B");
    mkdirSync(join(projA, ".agentctl"), { recursive: true });
    mkdirSync(join(projB, ".agentctl"), { recursive: true });
    process.env.AGENTCTL_SOCK = join(projB, ".agentctl", "relay.sock");

    // project-A has a control plane dir but no socket yet: its OWN path, not B.
    expect(socketPathFor(projA)).toBe(join(projA, ".agentctl", "relay.sock"));

    // An existing socket deeper down wins, walking up from a subdirectory.
    writeFileSync(join(projA, ".agentctl", "relay.sock"), "");
    const sub = join(projA, "src", "deep");
    mkdirSync(sub, { recursive: true });
    expect(socketPathFor(sub)).toBe(join(projA, ".agentctl", "relay.sock"));

    // A directory with no .agentctl anywhere: fail closed (NEVER project-B).
    const projC = join(root, "project-C");
    mkdirSync(projC, { recursive: true });
    expect(socketPathFor(projC)).toBeNull();
  });

  test("AGENTCTL_SOCK is only a fallback in dedicated mode when the directory is unknown", () => {
    process.env.AGENTCTL_SOCK = "/some/dedicated.sock";
  ...
  test("a known directory with no control plane yields no socket (request fails closed)", async () => {
    ...
    process.env.AGENTCTL_SOCK = "/elsewhere/relay.sock";
...
describe("auto-attach is request/response", () => {
  ...
  test("a rejected attach is retried after the cooldown and binds only on ok+managed", async () => {
    ...
    mkdirSync(join(proj, ".agentctl"), { recursive: true });
    const sockPath = join(proj, ".agentctl", "relay.sock");
